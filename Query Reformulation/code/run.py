...
def test(args, model, tokenizer):
    ...
    topK = int(args.topK)
    with open(os.path.join(args.output_dir,"predictions_"+ args.test_data_file[-18:-6] +".jsonl"),'w') as f:
        for index,url,sort_id in zip(indexs,urls,sort_ids):
            js={}
            js['url']=url
            js['answers']=[]
            step = 0
            for idx in sort_id[:topK * 100]:
                if step % topK == 0:
...
def main():
    ...
    parser.add_argument('--topK', type=int, default=1, help="For generating multiple results.")
